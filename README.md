...
Task runs consume quota on the accounts you enable for automation and follow the same rotation settings as normal proxy traffic: the strategy chosen in General settings (priority or round-robin), each account's priority, and the pre-emptive usage thresholds. When a task has several allowed accounts, runs prefer whichever still has headroom and rotate away from an account before it hard-limits. The **May consume banked window** switch controls whether automation may spend a reset that has not started yet.
